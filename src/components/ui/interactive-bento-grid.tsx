...
// Interactive Bento Grid Component
interface BentoItem {
  title: string;
  description: string;
  type: 'analytics' | 'wave' | 'geometric' | 'network';
  gradient?: string;
}
...
export function InteractiveBentoGrid({ items }: InteractiveBentoGridProps) {
  const getVisual = (type: BentoItem['type']) => {
    switch (type) {
      case 'analytics':
